...
Set every Intersight server's **UserLabel** to match the **Name of its assigned Server Profile**, so the label you see in the inventory always reflects the profile (and therefore the workload) currently running on that hardware.
...
| Account role | At minimum **Server Administrator** for the target org(s), or any role with **Update** on `compute.Blade` and `compute.RackUnit` plus **Read** on `server.Profile` and `organization.Organization`. **Read Only** roles will return 403 on the update calls. | A 403 mid-run on a specific server simply gets logged as Failed; the rest of the batch continues. |
| Network | Outbound HTTPS 443 to `intersight.com` (or your appliance FQDN) | Cisco VPN may be required if corporate egress restricts external destinations. Verify with `Test-NetConnection intersight.com -Port 443`. |
...
2. **Generate an Intersight API key** in the UI (**Settings -> API Keys -> Generate API Key**, ECDSA P-256 + SHA256). Save the secret PEM somewhere local, never in git. Suggested:

   - Personal use: `personal\intersight-secret.pem` (the `personal\` tree is gitignored)
   - Customer use: `customers\<Customer>\secrets\intersight-secret.pem` (the `customers\` tree is gitignored)
...
- Export the current UserLabel state for a quick backup you can restore from:
  ...
  To restore, write a small companion script that reads the CSV and calls `Set-IntersightComputeBlade` / `Set-IntersightComputeRackUnit` with the saved `UserLabel`.
...
- `2026-06-15` - added cross-link to the companion restore script.
